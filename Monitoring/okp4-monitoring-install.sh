...
echo ''
echo -e 'INSTALLING OKP4  NODE MONITORING'
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/okp4d or /home/user/go/bin/okp4d
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: okp4valoper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
[[outputs.influxdb]]
  database = "okp4metricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/okp4-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_okp4.conf /etc/telegraf/telegraf_okp4.conf
...
echo ''
if [[ `sudo systemctl status telegraf_okp4 | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_okp4 -f\e[0m'
